...
class CoarseDecoder(nn.Module):
    def __init__(self, output_channels=3, bilinear=False):
        super(Decoder, self).__init__()
        self.cf1 = CompressFeatures(128, 64)
        self.block9 = ResNetBlock(64) 
        self.cf2 = CompressFeatures(64, 32)
        self.block11 = ResNetBlock(32) 
        self.cf3 = CompressFeatures(32, 16)
        ...
        if bilinear:
            ...
            self.up1 = UpsamplingDeconv3d()
            self.up2 = UpsamplingDeconv3d()
            self.up3 = UpsamplingDeconv3d()
        self.sig = nn.Sigmoid()

    def forward(self, x):
        sp3 = x['spatial_level_3'] + self.up1(self.cf1(x['spatial_level_4']))
        sp3 = self.block9(sp3)
        sp2 = x['spatial_level_2'] + self.up2(self.cf2(sp3))
        sp2 = self.block11(sp2)
        sp1 = x['spatial_level_1'] + self.up3(self.cf3(sp2))
...
class DeconvDecoder(nn.Module):
    def __init__(self, output_channels=3):
        ...
        self.up43 = UpsamplingDeconv3d()
        self.up32 = UpsamplingDeconv3d()
        self.up21 = UpsamplingDeconv3d()

        self.up = nn.Upsample(scale_factor=2, mode='nearest')
        self.sig = nn.Sigmoid()

    def forward(self, x):
        sp3 = x['spatial_level_3'] + self.up43(self.cf1(x['spatial_level_4']))
        sp3 = self.block9(sp3)
        sp2 = x['spatial_level_2'] + self.up32(self.cf2(sp3))
        sp2 = self.block11(sp2)
        sp1 = x['spatial_level_1'] + self.up21(self.cf3(sp2))
        sp1 = self.block13(sp1)
        output = self.sig(self.cf_final(sp1))
